    ...
    internal class VRPenCore
    {
        ...
        public void Clear()
        {
            foreach (var strokeMesh in _strokeMeshes)
            {
                ...
                {
                    // 메모리 해제
                    UnityEngine.Object.DestroyImmediate(strokeMesh.Mesh);
                }
            }
            _strokeMeshes.Clear();
            _currentMesh = null;
        ...
        // --- [Undo 메서드 - 마지막 스트로크 제거] ---
        public void Undo()
        {
            if (_strokeMeshes.Count > 0)
            {
                // 1. 마지막 스트로크를 가져와서 제거합니다.
                var lastStrokeMesh = _strokeMeshes[^1];
                _strokeMeshes.RemoveAt(_strokeMeshes.Count - 1);

                // 2. (중요) 그래픽 메모리에서 해당 메쉬 객체를 정리합니다.
                if (lastStrokeMesh.Mesh != null)
                {
                    UnityEngine.Object.DestroyImmediate(lastStrokeMesh.Mesh);
                    lastStrokeMesh.Mesh = null;
                }

                // 3. 현재 그리는 중이던 스트로크가 제거되었다면, null로 설정
                if (_currentMesh == lastStrokeMesh)
                {
                    _currentMesh = null;
                }
            }
